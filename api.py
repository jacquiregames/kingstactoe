...
from schemas import (
    ...
    AddBotRequest, RemoveBotRequest
)
...
async def pass_turn_endpoint(request: PlayerRequest):
    async with state.state_lock:
        game = state.get_game()
        if request.player_name != game.get_current_player(): raise HTTPException(400, "Not your turn.")
        if game.pending_draw_tile.get(request.player_name): raise HTTPException(400, "You must place your drawn tile.")
        game._add_log_entry(f"{request.player_name} passed their turn.")
        game.advance_turn()
